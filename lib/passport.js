    ...
    async (req, email, password, done) => {
      ...
      if (result.rows.length > 0) {
        ...
        if (validPassword) {
          ...
          await pool.query(
            "UPDATE users SET fecha_ult_se = $1, log_token = $2 WHERE email = $3",
            [fecha, log_token, result.rows[0].email]
          );
          ...
          try {
            if (result.rows[0].tipusu == 2) {
              done(null, {"email": result.rows[0].email, "tipsus": 2});
              console.log("recluter");
            } else {
      ...
      if (result_email.rows.length === 0) {
        ...
        try {
          const result = await pool.query(
            "INSERT INTO users (tipUsu, nom_usu, email, password, fecha_ult_se, log_token) VALUES (1, $1, $2, $3, $4, $5)",
            [
              newUser.nom_usu,
              newUser.email,
              newUser.passwordcry,
        ...
        return done(
          null,
          false,
          req.flash("message", "Este correo ya esta resgitrado")
        );
      }
    }
  )
