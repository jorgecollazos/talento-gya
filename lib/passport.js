...
passport.use('local.signin', new LocalStrategy({
    ...
    if (result.rows.length > 0) {
        ...
        if (validPassword) {
            await pool.query('UPDATE users SET fecha_ult_se = $1 WHERE email = $2', [fecha, result.rows[0].email]);
...
passport.use('local.signup', new LocalStrategy({
    ...
    if (result_email.rows.length === 0) {
        try {
            const result = await pool.query("INSERT INTO users (tipUsu, nom_usu, email, password, fecha_ult_se) VALUES (1, $1, $2, $3, $4)", [newUser.nom_usu, newUser.email, newUser.passwordcry, newUser.fecha]);
